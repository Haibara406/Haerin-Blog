  ...
  useEffect(() => {
    if (loop) {
      const timer = setTimeout(() => {
        setKey(prev => prev + 1)
      }, (duration + 2) * 1000) // 动画时长 + 2秒暂停

      return () => clearTimeout(timer)
    }
  }, [key, loop, duration])

  // 统一使用英文版本 "hello" - 使用真实的 Apple Hello SVG 路径
  return (
      ...
      >
        ...
        >
          ...
          <motion.path
            ...
            transition={{
              pathLength: { duration: duration * 0.3, ease: [0.4, 0, 0.2, 1] },
          ...
          <motion.path
            ...
            animate={{ pathLength: 1, opacity: 1 }}
            transition={{
              pathLength: { duration: duration * 0.7, ease: [0.4, 0, 0.2, 1], delay: duration * 0.3 },
              opacity: { duration: 0.3, delay: duration * 0.3 }
            }}
